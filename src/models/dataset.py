...
def _next_day_returns_per_symbol(symbols: list[str]) -> pd.DataFrame:
    """For each symbol we need close[T+1] / close[T] - 1 keyed at T.
...
def build_training_matrix(
    ...
    *,
    ...
    fwd = _next_day_returns_per_symbol(syms_present)
    ...
    log.info(
        "Built matrix: {} rows, {} features, positive_rate={:.3f}",
        len(X),
        len(feature_cols),
